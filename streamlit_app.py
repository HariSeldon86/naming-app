...
compound_word = st.text_input("Enter a compound word (e.g., AdasVehicleSpeed):")
...
selected_parts = []
for i in range(3):  # Assuming max 3 parts for simplicity, adjust as needed
    selected_parts.append(st.selectbox(f"Part {i+1}", [""] + list(keyword_dict.keys()), key=f"part_{i+1}"))

if all(selected_parts):
    composed_word = ''.join(selected_parts)
    composed_abbreviations = get_abbreviations(selected_parts)
...
if compound_word:
    st.write("### Corresponding Dropdown Selections:")
    for i, part in enumerate(parts):
        st.write(f"Part {i+1}: {part}")

    st.write("### Abbreviations for Dropdown Selections:")
    for i, abbreviation in enumerate(abbreviations):
        st.write(f"Part {i+1}: {abbreviation}")

elif all(selected_parts):
    st.write("### Corresponding Compound Word:")
    st.write(composed_word)

    st.write("### Abbreviations for Selected Parts:")
